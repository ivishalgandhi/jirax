...
def main():
    ...
    # Test case 8: Test issue type filter (new)
    cmd8 = f"source .venv/bin/activate && PYTHONPATH=. python jirax.py extract --query \"project = {test_project} AND issuetype = Task\" --output-path {TEST_OUTPUT_DIR}/test8_{timestamp}.csv --no-preview"
    results.append(run_test("Query with issue type filter", cmd8))
    ...
    csv_files = [
        ...
        f"{TEST_OUTPUT_DIR}/test8_{timestamp}.csv"
    ]
